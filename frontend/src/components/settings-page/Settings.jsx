...
const Settings = () => {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.put('https://shorten-url-new.onrender.com/api/users/update', {
        username: name,
        email,
        phone: mobile
      }, {
  ...
  const handleDeleteAccount = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete('https://shorten-url-new.onrender.com/api/users/delete', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
